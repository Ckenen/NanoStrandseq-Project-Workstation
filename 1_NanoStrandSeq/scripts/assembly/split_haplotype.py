...
def calculate_bin_read_counts(chrom, length, segments, bin_width):
...
def main():
    ...
    cell = os.path.splitext(os.path.basename(infile))[0]
    count_dir = outdir + "/counts"
    bam_dir = outdir + "/bams"
    pdf_dir = outdir + "/pdfs"
    make_directory(outdir)
    make_directory(count_dir)
    make_directory(bam_dir)
    ...
    chrom_segments_rmdup = defaultdict(list)
    for segment in segments:
        if segment.get_tag("BH") != "Y":
            segments_nobh.append(segment)
            chrom = segment.reference_name
            chrom_segments[chrom].append(segment)
            if not segment.is_duplicate:
                chrom_segments_rmdup[chrom].append(segment)

    logging.info("loaded %d chromosomes" % len(chroms))
    logging.info("loaded %d segments, include %d no blackhole segments" % (len(segments), len(segments_nobh)))
    logging.info("-" * 80)
    ...
    for chrom in chroms:
        ...
        counts1 = calculate_bin_read_counts(chrom, lengths[chrom], chrom_segments[chrom], bin_width)
        chrom_counts[chrom] = counts1
        counts1.to_csv(count_dir + "/%s.tsv" % chrom, sep="\t", index=False)
        counts2 = calculate_bin_read_counts(chrom, lengths[chrom], chrom_segments_rmdup[chrom], bin_width)
    ...
    for chrom in chroms:
        ...
        d2 = calculate_bin_read_counts(chrom, lengths[chrom], segments_selected, bin_width)
        d3 = calculate_bin_read_counts(chrom, lengths[chrom], segments_hp1, bin_width)
        d4 = calculate_bin_read_counts(chrom, lengths[chrom], segments_hp2, bin_width)
        ...
        if True: # output bam
            ...
            obam1 = bam_dir + "/%s.hp1.bam" % chrom
            obam2 = bam_dir + "/%s.hp2.bam" % chrom
            with pysam.AlignmentFile(obam1, "wb", header=header) as fw:
                for segment in items1:
                    fw.write(segment)
            with pysam.AlignmentFile(obam2, "wb", header=header) as fw:
                for segment in items2:
                    fw.write(segment)
            logging.info("wrote %s segments for %s haplotype 1" % (len(items1), chrom))
            logging.info("wrote %s segments for %s haplotype 2" % (len(items2), chrom)) 
            logging.info("generating bam index")
            assert os.system("samtools index %s" % obam1) == 0
            assert os.system("samtools index %s" % obam2) == 0
