...
def main():
    ...
    with open(outfile + "_reference_only.bed", "w+") as fw:
        for chrom, start, a1, a2 in sorted(hets1 - hets3):
            fw.write("%s\t%d\t%d\t%s\n" % (chrom, start, start + 1, "%s/%s" % (a1, a2)))
    with open(outfile + "_query_only.bed", "w+") as fw:
        for chrom, start, a1, a2 in sorted(hets2 - hets3):
            fw.write("%s\t%d\t%d\t%s\n" % (chrom, start, start + 1, "%s/%s" % (a1, a2)))
    with open(outfile + "_common.bed", "w+") as fw:
        for chrom, start, a1, a2 in sorted(hets3):
            fw.write("%s\t%d\t%d\t%s\n" % (chrom, start, start + 1, "%s/%s" % (a1, a2)))
