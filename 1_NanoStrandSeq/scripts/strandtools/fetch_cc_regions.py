...
class FetchCCRegion(object):
    ...
    def make_data_frame(cls, data, chrom, outdir):
        ...
        for cell in cells:
            ...
            d = data[cell]
            d = d[d["Chrom"] == chrom]
            counts1 = d["Crick"].values
            counts2 = d["Watson"].values 
    ...
    def execute(self):
        ...
        for cell in config["Cells"]:
            # run = cell.split(".")[0]
            # path = "../1_NanoStrandseq/results/barplot/stat_bin_reads/%s/%s.RmDup0.RmLC0.tsv" % (run, cell)
            path = "./results/%s/prepare/stat_bin_reads/%s.RmDup0.tsv" % (config["Name"], cell)
