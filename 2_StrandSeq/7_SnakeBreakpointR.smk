...
rule breakpointR:
    ...
    shell:
        ...
        ln -s ${{bam}} {output.out}.tmp/${{name}}
        ln -s ${{bam}}.bai {output.out}.tmp/${{name}}.bai
        set +u; source activate breakpointR
        ./scripts/breakpointR.PE.R {output.out}.tmp {threads} {output.out} 
        rm -r {output.out}.tmp
...
rule get_lib_metrics:
    ...
    shell:
        """(
        set +u; source activate breakpointR
        ./scripts/get_lib_metrics.R {input}/data/{wildcards.cell}.bam.RData {output.txt}
        conda deactivate ) &> {log}
        """