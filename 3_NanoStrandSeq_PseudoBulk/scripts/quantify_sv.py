...
def quantify_chrom_sv(f_vcf, f_bam, chrom, outfile):
...
def main():
    parser = optparse.OptionParser(usage="%prog [options] sv.vcf.gz aligned.bam")
    ...
    with pysam.VariantFile(f_vcf) as f:
        for chrom in f.header.contigs:
            ...
            pool.apply_async(quantify_chrom_sv, (f_vcf, f_bam, chrom, f_tsv))
            f_tsv_list.append(f_tsv)
    pool.close()
    pool.join()

    fw = sys.stdout if outfile is None else open(outfile, "w+")
